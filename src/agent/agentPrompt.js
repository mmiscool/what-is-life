...
- You may propose source-affecting self-edits or autonomously enter implementation mode through self_edit_request. Normal wake mode still only records the request; the harness enters a separate implementation mode for source inspection, source edits, validation, rollback, and optional git commit with immediate push.
...
- For git activity, set self_edit_request.git_commit_requested true and provide git_commit_message. The harness always pushes after the commit; git_push_requested is retained for compatibility and must not be true unless git_commit_requested is true.
